		...
		if k == "vendingmachine" and Config.Overrides.VendOverride then
			...
			for l, b in pairs(v["coords"]) do
				...
				local options = { {
					...
					label = (v["targetLabel"] or "Explorar Loja"),
					item = v["requiredItem"],
					job = v["job"] or nil,
					gang = v["gang"] or nil,
					action = function()
				...
				if k == "casino" then
					options[#options+1] = {
						...
						icon = "fab fa-galactic-republic", label = "Trocar Fichas ($"..Config.Overrides.SellCasinoChips.pricePer.." per chip)",
					}
				end
				if Config.Overrides.Peds then
...
RegisterNetEvent('jim-shops:ShopMenu', function(data, custom)
	...
	for i = 1, #products do
		...
		if not Items[products[i].name:lower()] then
			...
			if products[i].price == 0 then price = "Gratís" else price = "Preço: $"..products[i].price end

			local text = ""
			if products[i].info and products[i].info.item then text = text.."Item: "..products[i].info.item..br end
			text = text..price..br.."Peso: "..(Items[products[i].name].weight / 1000)..Config.Overrides.Measurement
			...
			if Config.Overrides.Limit and not custom then
				if stashItems[i] then
					...
					if amount ~= 0 then
						text = price..br.."Quantidade: x"..amount..br.."Peso: "..(Items[products[i].name].weight / 1000)..Config.Overrides.Measurement
					elseif amount == 0 then
						text = price..br.."Out Of Stock"..br.."Peso: "..(Items[products[i].name].weight / 1000)..Config.Overrides.Measurement
					end
				else
					text = price..br.."Out Of Stock"..br.."Peso: "..(Items[products[i].name].weight / 1000)..Config.Overrides.Measurement
					lock = true
